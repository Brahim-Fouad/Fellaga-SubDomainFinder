...
Fellaga 0.9.0 registers 30 passive connectors. It also has separate opportunistic direct CT-log monitoring, authoritative AXFR, DNS graph, DNSSEC, Web, TLS, and active DNS candidate generation; those mechanisms are not counted as passive connectors.
...
```bash
export FELLAGA_USER_AGENT='Fellaga/0.9.0 (security-team@example.org)'
```
