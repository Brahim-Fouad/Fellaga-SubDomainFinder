...
Fellaga v0.9.2 contains seven assets:
...
' fellaga-v0.9.2-x86_64-unknown-linux-gnu.cdx.json
```
...
gh attestation verify \
  fellaga-v0.9.2-x86_64-unknown-linux-gnu.tar.gz \
...
cosign verify-blob \
  ...
  --certificate-identity "https://github.com/Brahim-Fouad/Fellaga-SubDomainFinder/.github/workflows/release.yml@refs/tags/v0.9.2" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  SHA256SUMS
```

The certificate identity binds the keyless signature to the release workflow at the exact `v0.9.2` tag.
...
git rev-parse 'v0.9.2^{commit}'
git show --no-patch --format=fuller v0.9.2
```
