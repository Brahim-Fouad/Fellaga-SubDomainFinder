...
The manifest binds the campaign ID, repository commit, executable hashes and versions, input hashes, credential mode, resolver set, and DNS controls. The runner refuses an existing output directory so an earlier artifact cannot be reused accidentally.
...
- `FELLAGA_BENCH_DNS_RATE`: shared DNS request limit for tools that expose a strict QPS control, default `100`.
